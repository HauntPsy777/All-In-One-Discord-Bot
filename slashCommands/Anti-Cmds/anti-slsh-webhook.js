...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setDescription("Enable or disable the anti-webhook feature and set a log channel.")
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addStringOption(option =>
      option
        .setName("state")
        .setDescription("Turn the anti-webhook feature ON or OFF")
        .setRequired(true)
        .addChoices(
          { name: "On", value: "on" },
          { name: "Off", value: "off" }
    ...
    try {
      ...
      if (!logChannel.permissionsFor(interaction.client.user).has(PermissionFlagsBits.SendMessages)) {
        return interaction.reply({
          embeds: [
            new EmbedBuilder()
              .setDescription(`⚠️ **I don't have permission to send messages in ${logChannel}. Please fix this before enabling Anti-Webhook.**`)
              .setColor("Yellow"),
          ],
          ephemeral: true,
        });
      ...
      await antiWebhookDB.saveSettings(interaction.guild.id, isEnabled, logChannel.id);

      console.log(`✅ Anti-Webhook for Guild ${interaction.guild.id} set to ${option.toUpperCase()}, Log Channel: ${logChannel.id}`);

      const setupEmbed = new EmbedBuilder()
        .setTitle("🔒 **Anti-Webhook System Setup**")
        .setDescription(`Anti-webhook protection has been **${option.toUpperCase()}**.`)
        .addFields(
