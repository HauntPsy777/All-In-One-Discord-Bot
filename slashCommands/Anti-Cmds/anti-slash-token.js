...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("anti-token")
    .setDescription("Enable or disable the anti-token system and set up a log channel.")
    .setDefaultMemberPermissions(PermissionFlagsBits.Administrator)
    .addStringOption(option =>
      option
        .setName("state")
        .setDescription("Enable or disable the anti-token system")
        .setRequired(true)
        .addChoices(
          { name: "On", value: "on" },
          { name: "Off", value: "off" }
    ...
    try {
      ...
      if (!logChannel.permissionsFor(interaction.client.user).has(PermissionFlagsBits.SendMessages)) {
        return interaction.reply({
          embeds: [
            new EmbedBuilder()
              .setDescription(`⚠️ **I don't have permission to send messages in ${logChannel}. Please fix this before enabling Anti-Token.**`)
              .setColor("Yellow"),
          ],
          ephemeral: true,
        });
      ...
      await antiTokenDB.saveSettings(interaction.guild.id, isEnabled, logChannel.id);

      console.log(`✅ Anti-Token for Guild ${interaction.guild.id} set to ${option.toUpperCase()}, Log Channel: ${logChannel.id}`);

      const setupEmbed = new EmbedBuilder()
        .setTitle("🔒 **Anti-Token System Setup**")
        .setDescription(`Anti-token protection has been **${option.toUpperCase()}**.`)
        .addFields(
