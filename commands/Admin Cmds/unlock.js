    ...
    async execute(message, args) {
        ...
        try {
            ...
            await channel.permissionOverwrites.edit(message.guild.roles.everyone, {
                SendMessages: null // Restores default permission for sending messages
            });
            ...
            // Log the channel unlocking in a specified log channel
            const logChannel = message.guild.channels.cache.find(ch => ch.name === 'mod-logs');
            if (logChannel) {
                const logEmbed = new EmbedBuilder()
                    .setTitle('Channel Unlocked')
                    .setDescription(`
**Unlocked By:** ${message.author} (${message.author.id})
**Channel:** ${channel.name} (${channel.id})
            ...
            const errorEmbed = new EmbedBuilder()
                .setColor('Red')
                .setDescription('❌ An error occurred while trying to unlock the channel. Please try again later.');
