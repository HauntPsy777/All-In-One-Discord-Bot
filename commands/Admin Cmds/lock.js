...
module.exports = {
    ...
    async execute(message, args) {
        ...
        try {
            ...
            // Log the channel locking in a specified log channel
            const logChannel = message.guild.channels.cache.find(ch => ch.name === 'mod-logs');
            if (logChannel) {
                const logEmbed = new EmbedBuilder()
                    .setTitle('Channel Locked')
                    .setDescription(`
**Locked By:** ${message.author} (${message.author.id})
**Channel:** ${channel.name} (${channel.id})
            ...
            const errorEmbed = new EmbedBuilder()
                .setColor('Red')
                .setDescription('❌ An error occurred while trying to lock the channel. Please try again later.');
